class Artist
  ...
  def load
    ...
    if not_have_artist(clean_artist)
      search_and_save_artist(clean_artist)
    else
      find_artist(clean_artist)
    end
  end
  ...
  def not_have_artist(clean_artist)
    redis = Redis.new
    redis.get(clean_artist + ':artist').nil?
  end

  def find_artist(clean_artist)
  ...
  def search_and_save_artist(clean_artist)
